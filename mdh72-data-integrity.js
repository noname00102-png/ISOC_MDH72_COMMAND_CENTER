...
function install(){const frame=getFrame();if(!frame||frame.dataset.mdhIntegrityInstalled==='1')return false;const d=frame.contentDocument,w=frame.contentWindow;if(!d||!w)return false;frame.dataset.mdhIntegrityInstalled='1';
  ...
  d.addEventListener('click',async e=>{const b=e.target.closest('#saveButton');if(!b)return;const edit=d.getElementById('editId')?.value.trim();if(edit)return;
    ...
    try{const client=w.supabase?.createClient?null:null;const sb=window.supabase.createClient(SUPABASE_URL,SUPABASE_KEY,{auth:{persistSession:false,autoRefreshToken:false,detectSessionInUrl:false}});const since=new Date(Date.now()-DUP_WINDOW_MS).toISOString();const {data,error}=await sb.from('threats').select('id,created_at').eq('title',title).eq('threat_type',type).eq('district',district||null).eq('subdistrict',sub||null).gte('created_at',since).limit(5);
